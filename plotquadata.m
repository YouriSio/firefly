...
if option.plot_lambda
    ...
    legend({'\theta'}), xlabel('time'), ylabel('\theta');
    grid on;
    subplot(3,1,3);
...
if option.plot_omega
    ...
    legend({'\theta dot'}), xlabel('time'), ylabel('\theta dot');
    grid on;
    subplot(3,1,3);
...
if option.plot_u
    ...
    legend({'torque \tau\phi'}), xlabel('time'), ylabel('\tau\phi');
    grid on;
    subplot(4,1,3);
    plot(simlogdata.time,simlogdata.signals.values(:,15));
    legend({'torque \tau\theta'}), xlabel('time'), ylabel('\tau\theta');
    grid on;
    subplot(4,1,4);
    plot(simlogdata.time,simlogdata.signals.values(:,16));
    legend({'torque \tau\psi'}), xlabel('time'), ylabel('\tau\psi');
    grid on;
